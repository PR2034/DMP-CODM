            ...
            foreach (var imageDef in metadata.imageDefs)
            {
                ...
                for (var index = imageDef.typeStart; index < typeEnd; ++index)
                {
                    ...
                    if (addToken)
                    {
                        ...
                        customTokenAttribute.Fields.Add(new CustomAttributeNamedArgument("Token", new CustomAttributeArgument(stringType, $"0x{0:X}")));//assume token is 0, bcz token is removed from codm
            ...
            foreach (var imageDef in metadata.imageDefs)
            {
                ...
                for (int index = imageDef.typeStart; index < typeEnd; index++)
                {
                    ...
                    for (var i = typeDef.fieldStart; i < fieldEnd; ++i)
                    {
                        ...
                        {
                            var customTokenAttribute = new CustomAttribute(typeDefinition.Module.ImportReference(tokenAttribute));
                            customTokenAttribute.Fields.Add(new CustomAttributeNamedArgument("Token", new CustomAttributeArgument(stringType, $"0x{0:X}")));//assume token is 0 //fieldDef.token 
                    ...
                    for (var i = typeDef.propertyStart; i < propertyEnd; ++i)
                    {
                        ...
                        {
                            var customTokenAttribute = new CustomAttribute(typeDefinition.Module.ImportReference(tokenAttribute));
                            customTokenAttribute.Fields.Add(new CustomAttributeNamedArgument("Token", new CustomAttributeArgument(stringType, $"0x{0:X}")));//propertyDef.token
                    ...
                    for (var i = typeDef.eventStart; i < eventEnd; ++i)
                    {
                        ...
                        {
                            var customTokenAttribute = new CustomAttribute(typeDefinition.Module.ImportReference(tokenAttribute));
                            customTokenAttribute.Fields.Add(new CustomAttributeNamedArgument("Token", new CustomAttributeArgument(stringType, $"0x{0:X}")));//eventDef.token
            ...
            if (il2Cpp.Version > 20)
            {
                foreach (var imageDef in metadata.imageDefs)
                {
                    ...
                    for (int index = imageDef.typeStart; index < typeEnd; index++)
                    {
                        ...
                        var typeDefinition = typeDefinitionDic[typeDef];
                        //typeAttribute
                        CreateCustomAttribute(imageDef, typeDef.customAttributeIndex, 0, typeDefinition.Module, typeDefinition.CustomAttributes);
                        ...
                        for (var i = typeDef.fieldStart; i < fieldEnd; ++i)
                        {
                            ...
                            //fieldAttribute
                            CreateCustomAttribute(imageDef, fieldDef.customAttributeIndex, 0, typeDefinition.Module, fieldDefinition.CustomAttributes);
                        }
                        ...
                        for (var i = typeDef.methodStart; i < methodEnd; ++i)
                        {
                            ...
                            for (var j = 0; j < methodDef.parameterCount; ++j)
                            {
                                ...
                                //parameterAttribute
                                CreateCustomAttribute(imageDef, parameterDef.customAttributeIndex, 0, typeDefinition.Module, parameterDefinition.CustomAttributes);
                            }
                        }
                        ...
                        for (var i = typeDef.propertyStart; i < propertyEnd; ++i)
                        {
                            ...
                            //propertyAttribute
                            CreateCustomAttribute(imageDef, propertyDef.customAttributeIndex, 0, typeDefinition.Module, propertyDefinition.CustomAttributes);
                        }
                        ...
                        for (var i = typeDef.eventStart; i < eventEnd; ++i)
                        {
                            ...
                            //eventAttribute
                            CreateCustomAttribute(imageDef, eventDef.customAttributeIndex, 0, typeDefinition.Module, eventDefinition.CustomAttributes);
                        }
                    }
                }
            }
